...
class UserService {
  ...
  async activate(activationLink) {
    ...
    const user = await db.query(
      "SELECT EXISTS (SELECT 1 FROM users WHERE activation_link = $1)",
      [activationLink]
    );
    ...
    const activate_true = await db.query(
  ...
  async login(email, password) {
    ...
    const passwordFromDb = await db.query(
    ...
    const isPassEquals = await bcrypt.compare(
      password,
      passwordFromDb.rows[0].password
    );
    if (!isPassEquals) {
      throw ApiError.BadRequest("Неверный пароль");
    }

    if (!passwordFromDb.rows[0].is_activated) {
      throw ApiError.BadRequest("Регистрация не подтверждена");
    }
    const userObj = {
      email,
      password,
      isActivated: passwordFromDb.rows[0].is_activated,
